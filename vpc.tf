...
resource "aws_route" "private_nat_gateway" {
  ...
  count                  = "${length(var.azs) * lookup(map(var.enable_nat_gateway, 1), 1, 0)}"
}
...
resource "aws_subnet" "private" {
  ...
  tags              = "${merge(var.tags, map("Name", format("%s-subnet-private-%s-%s", var.name, element(var.private_subnet_names, count.index), element(var.azs, count.index))))}"
}
